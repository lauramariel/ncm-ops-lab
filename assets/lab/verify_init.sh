...
echo "Checking for OpsDemoCluster in https://127.0.0.1:9440/api/nutanix/v3/clusters/list"
resp=`curl -s -k -X POST https://127.0.0.1:9440/api/nutanix/v3/clusters/list -H 'Content-type: application/json' -d '{ "filter":"" }' --user $user:$password | jq -r '.entities[].status | select(.name == "OpsDemoCluster")'`
echo "Result: $resp"
...
while [[ -z $resp ]];
do
echo "Attempt $i .. OpsDemoCluster doesn't exist in clusters/list, running create_zeus_entity.py again"
...
# If we're here, that means OpsDemoCluster exists
echo "OpsDemoCluster exists in clusters/list"
